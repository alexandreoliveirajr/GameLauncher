...
class GameManager:
    ...
    def get_executables_for_game(self, game_id):
        """Busca todos os executáveis associados a um ID de jogo."""
        conn = get_db_connection()
        try:
    ...
    def _get_base_game_query_fields(self, alias=None):
        ...
        # --- INÍCIO DA CORREÇÃO ---
        # Removido os aliases "as image" e "as background" para usar os nomes de colunas corretos.
        fields = [
    ...
    def update_game(self, old_game_data, new_game_data):
        ...
        # --- INÍCIO DA CORREÇÃO ---
        # Usando as chaves corretas para buscar os dados de imagem.
        cursor.execute("""
            UPDATE games SET
        ...
        """, (
            ...
            new_game_data.get('image_path', old_game_data.get('image_path')),
            new_game_data.get('background_path', old_game_data.get('background_path')),
            new_game_data.get('header_path', old_game_data.get('header_path')),
    ...
    def get_filtered_games(self, search_text, tag=None, sort_by="Nome (A-Z)"):
        ...
        if sort_by == "Nome (A-Z)":
            ...
            query += " ORDER BY g.playtime_local DESC"
        elif sort_by == "Jogado Recentemente":
            query += " ORDER BY g.last_played_timestamp DESC"
    ...
    def toggle_favorite(self, game_id):
        conn = get_db_connection()
        # Primeiro, busca o status atual de favorito
    ...
    def update_last_played(self, game_id):
        """Atualiza o timestamp da última vez que um jogo foi jogado para o momento atual."""
        conn = get_db_connection()
        try:
    ...
    def add_or_update_steam_game(self, app_id, name, install_dir, status):
        """
        Adiciona ou atualiza um jogo da Steam, garante que o executável exista,
        e baixa as artes se necessário de forma robusta.
        """
        conn = get_db_connection()
        try:
            cursor = conn.cursor()
            game = cursor.execute("SELECT id FROM games WHERE source = 'steam' AND app_id = ?", (app_id,)).fetchone()
            if game:
                game_id = game['id']
            ...
            conn.commit()
            if game_id:
                art_check = cursor.execute("SELECT image_path FROM games WHERE id = ?", (game_id,)).fetchone()
                if not art_check or not art_check['image_path']:
                    logging.info(f"Buscando artes para o jogo '{name}' (AppID: {app_id})...")
                    artwork_paths = download_steam_artwork(app_id)
                    if artwork_paths:
                        cursor.execute(
                            """UPDATE games SET image_path = ?, header_path = ?, background_path = ? WHERE id = ?""",
                            (
                                artwork_paths.get('image_path'),
                                artwork_paths.get('header_path'),
                                artwork_paths.get('background_path'),
                                game_id
                            )
                        )
                        conn.commit()
