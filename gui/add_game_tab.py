...
class AddGameTab(QWidget):
    ...
    def _add_game(self):
        ...
        # ADICIONADO: Lógica para processar as tags
        tags_text = self.tags_input.text().strip()
        tags_list = []
        if tags_text: # Só processa se o campo não estiver vazio
            # Separa por vírgula e remove espaços extras de cada tag
            tags_list = [tag.strip() for tag in tags_text.split(',')]
        
        self.game_manager.add_game(game_name, self.current_paths, image_path, background_path, tags=tags_list)
